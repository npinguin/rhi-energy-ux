  ...
  class EnergyRuntime {
    ...
    row(key) {
      ...
      if (entityId) {
        ...
        if (hit) return { entity_id: entityId, ...hit, key };
      }
      return this.allRows().get(String(key)) || { key, value: null, unit: '', health: 'NOT_PUBLISHED', quality: 'missing', missing: true };
    }
    ...
    release() {
      ...
      return {
        ...
        backend_release: attrs.backend_version || attrs.backend_release || attrs.release || attrs.release_version || this.releaseState()?.state || 'unknown',
        contract_version: attrs.contract_version || attrs.schema_version || attrs.contract || 'unknown',
    ...
    overviewExperience() {
      const entityId = this.publicUxEntities().includes('sensor.energy_overview_experience')
        ? 'sensor.energy_overview_experience'
        : this.interfaceEntity('overviewExperience');
      const attrs = this.attrs(entityId);
    ...
    pilotReadiness() {
      const entityId = this.publicUxEntities().includes('sensor.energy_pilot_readiness') ? 'sensor.energy_pilot_readiness' : null;
      const attrs = this.attrs(entityId);
    ...
    planningExperienceRows() {
      ...
      const entityId = this.publicUxEntities().includes('sensor.energy_planning_experience_index')
        ? 'sensor.energy_planning_experience_index'
        : this.interfaceEntity('planningExperience');
      const attrs = this.attrs(entityId);
    ...
    consumerMixIndexEntity() {
      if (this.publicUxEntities().includes('sensor.energy_consumer_mix_index')) return 'sensor.energy_consumer_mix_index';
    ...
    flexibleAssetIndexRows() {
      ...
      const canonicalEntity = this.publicUxEntities().includes('sensor.energy_flexible_asset_index')
        ? 'sensor.energy_flexible_asset_index'
        : this.interfaceEntity('flexibleAssets');
    ...
    planningIndexRows() {
      ...
      const entityId = this.publicUxEntities().includes('sensor.energy_planning_index')
        ? 'sensor.energy_planning_index'
        : this.interfaceEntity('planning');
      const attrs = this.attrs(entityId);
    ...
    effectiveStrategyRows() {
      ...
      const entityId = this.publicUxEntities().includes('sensor.energy_strategy_effective_index')
        ? 'sensor.energy_strategy_effective_index'
        : this.interfaceEntity('strategyEffective');
      const attrs = this.attrs(entityId);
    ...
    outlookHorizons() {
      ...
      const entityId = this.publicUxEntities().includes('sensor.energy_outlook_property_index')
        ? 'sensor.energy_outlook_property_index'
        : this.interfaceEntity('outlook');
    ...
    meteringHorizons() {
      ...
      const entityId = this.publicUxEntities().includes('sensor.energy_metering_property_index')
        ? 'sensor.energy_metering_property_index'
        : this.interfaceEntity('metering');
    ...
    valuePeriods() {
      ...
      const entityId = this.publicUxEntities().includes('sensor.energy_value_accounting_index')
...
// END GENERATED MODULE: planning/planning-view-model.js
  ...
  class HomeBrainEnergyCard extends HTMLElement {
    constructor() {
      ...
      this.loadSort = 'priority';
      this.detailOpen = {};
      this.selectedStrategyProfileId = '';
      this.strategyEditProfileId = '';
      this.selectedOutlookHorizonId = 'D0';
      ...
      this.planningScrollLeft = 0;
      this.planningScrollTop = 0;
    ...
    syncMeteringPeriodFromRuntime() {
      ...
      if (['today','day','week','month','year'].includes(value)) this.selectedMeteringPeriodId = value === 'day' ? 'today' : value;
    }
    ...
    onClick(event) {
      ...
      if (consumerSort && !consumerSort.disabled) { this.consumerSort = consumerSort.dataset.consumerSort || 'cost'; this.render(); return; }
      ...
      if (scopePick && !scopePick.disabled) {
        ...
        if (context === 'metering-period') {
          this.selectedMeteringPeriodId = id || 'today';
          this.requestPropertyWrite('metering.selected_period', this.selectedMeteringPeriodId, { source: 'metering_period_selector' });
        }
        else if (context === 'metering') this.selectedMeteringHorizonId = id || 'D0';
        else if (context === 'value') {
          this.selectedMeteringPeriodId = id || 'today';
          this.requestPropertyWrite('metering.selected_period', this.selectedMeteringPeriodId, { source: 'value_period_selector' });
        }
    ...
    onChange(event) {
      ...
      if (consumerSortSelect && !consumerSortSelect.disabled) { this.consumerSort = consumerSortSelect.value || 'cost'; this.render(); return; }
      ...
      if (scopeSelect && !scopeSelect.disabled) {
        ...
        if (context === 'metering-period') this.selectedMeteringPeriodId = scopeSelect.value || 'today';
    ...
    componentScopeSelector({ context = 'outlook', title = 'Horizon', items = [], selectedId = '', idField = 'id', labelFn = item => human(item?.label || item?.id || '') }) {
      ...
      const active = normalized.find(row => row.id.toLowerCase() === String(selectedId || '').toLowerCase()) || normalized[0];
      const buttons = normalized.map(row => `<button type="button" class="scopeOption ${row.id === active.id ? 'active' : ''}" data-scope-context="${escapeHtml(context)}" data-scope-id="${escapeHtml(row.id)}">${escapeHtml(row.label)}</button>`).join('');
      const select = `<select data-scope-select data-scope-context="${escapeHtml(context)}">${normalized.map(row => `<option value="${escapeHtml(row.id)}" ${row.id === active.id ? 'selected' : ''}>${escapeHtml(row.label)}</option>`).join('')}</select>`;
      return `<div class="scopeSelector"><div class="scopeSelectorTitle"><span>${escapeHtml(title)}</span><b>${escapeHtml(active.label)}</b></div><div class="scopeButtons">${buttons}</div>${select}</div>`;
    }
    ...
    tabExperienceHeader(rt, tab, pageViewModel = null) {
      ...
      return `<section class="hiTabExperienceHeader ${escapeHtml(p.tone)}">
        <div class="hiTabHero" data-view="${escapeHtml(tab)}" style="--hi-hero-image:url('${escapeHtml(p.image || '')}')">
          ...
          <div class="hiTabHeroCopy"><small>${escapeHtml(p.eyebrow)}</small><h2>${escapeHtml(p.title)}</h2><div class="hiTabHeroValue">${escapeHtml(p.value)}</div><span>${escapeHtml(p.unit)}</span><p>${escapeHtml(p.explanation)}</p></div>
    ...
    outlook(rt) {
      ...
      this.selectedOutlookHorizonId = horizon?.horizon_id || 'D0';
    ...
    batteryChildCard(rt, assetId) {
      ...
      const health = rt.rawText(`${assetId}.health`, rt.row(`${assetId}.power_kw`).health || 'OK');
      const image = this.batteryHeroImagePath(assetId, name);
      return `<article class="batteryContributorCard"><div class="batteryContributorVisual"><img class="batteryContributorImage" src="${image}" alt="${escapeHtml(name)}"></div><div class="batteryContributorBody"><div class="batteryContributorHeader"><div><b>${escapeHtml(name)}</b><span class="batteryHealth">${escapeHtml(health)}</span></div><strong>${fmtPct(soc)}</strong></div><div class="batteryContributorMeta"><span>Power now</span><b>${fmtKw(power, '0.0 kW')}</b></div><div class="batteryContributorState"><span>${(power||0)>0.05?'Active':'Ready'}</span><small>${(power||0)>0.05?'Contributing now':'Available for the energy plan'}</small></div><div class="bar"><i style="width:${escapeHtml(this.progress(soc,100))}%"></i></div></div></article>`;
    }
    battery(rt) {
    ...
    consumerFallbackRows(rt) {
      const flexible = rt.primaryFlexibleAssets().map(asset => ({
        ...asset,
        display_name: asset.display_name || rt.assetName(asset.asset_id),
        category: /storage|battery/i.test(`${asset.flexible_role || ''} ${asset.asset_type || ''}`) ? 'storage' : (/vehicle|mobility/i.test(`${asset.asset_type || ''} ${asset.flexible_role || ''}`) ? 'vehicle' : (asset.asset_type || 'flexible load')),
        controllability: 'flexible',
        flexible: true,
        current_power_kw: firstDefined(asset.power_kw, asset.current_power_kw, rt.powerForAsset(asset.asset_id)),
        expected_energy_kwh: firstDefined(asset.energy_to_target_kwh, asset.energy_needed_kwh),
        entity_id: asset.entity_id || 'sensor.energy_flexible_asset_index'
      }));
      const ids = new Set(flexible.map(row => String(row.asset_id)));
      const legacy = rt.consumerRows().filter(row => !ids.has(String(row.asset_id))).map(row => ({
        ...row,
        category: row.asset_type || 'observed consumer',
        controllability: row.flexibility_available ? 'flexible' : 'observed',
        flexible: !!row.flexibility_available,
        entity_id: row.entity_id || UX_INTERFACES.consumer
      }));
    ...
    consumers(rt) {
      ...
      const rows = publishedRows.length ? publishedRows : this.consumerFallbackRows(rt);
    ...
    strategies(rt) {
      ...
      const selected = (this.selectedStrategyProfileId && rt.strategyProfileFor(this.selectedStrategyProfileId)) || fallbackProfile;
    ...
    diagnosticSpec(tab) {
      const specs = {
        overview: [
          ['sensor.energy_release_contract','Release compatibility','Publishes the interfaces and compatible version used by this screen'],
          ['sensor.energy_overview_experience','Overview explanation','Provides the product-safe overview summary'],
          ['sensor.energy_solar_property_index','Solar','Live solar production'],
          ['sensor.energy_consumption_property_index','Home demand','Current household consumption'],
          ['sensor.energy_grid_property_index','Grid','Import and export'],
          ['sensor.energy_battery_property_index','Home Battery','Home Battery state and flow'],
          ['sensor.energy_intelligence_property_index','Guidance','Recommendation and reason']
        ],
        outlook: [
          ['sensor.energy_release_contract','Release compatibility','Publishes the interfaces used by this screen'],
          ['sensor.energy_outlook_property_index','Outlook','Today and Tomorrow summaries'],
          ['sensor.energy_forecast_property_index','Forecast','Solar forecast facts'],
          ['sensor.energy_planning_index','Planning facts','Timing, targets and feasibility'],
          ['sensor.energy_planning_experience_index','Planning explanation','Product-safe wording and next action']
        ],
        planning: [
          ['sensor.energy_release_contract','Release compatibility','Publishes the interfaces used by this screen'],
          ['sensor.energy_planning_index','Planning timeline','Authoritative Today and Tomorrow planning buckets'],
          ['sensor.energy_flexible_asset_index','Flexible assets','Asset names, availability and energy need'],
          ['sensor.energy_outlook_property_index','Outlook projection','Projected graph and horizon context']
        ],
        retrospective: [
          ['sensor.energy_release_contract','Release compatibility','Publishes the interfaces used by this screen'],
          ['sensor.energy_retrospective_event_index','Retrospective','Canonical score, evidence coverage, prerequisites, objectives and recommendations']
        ],
        flow: [
          ['sensor.energy_release_contract','Release compatibility','Publishes the interfaces used by this screen'],
          ['sensor.energy_asset_index','Assets','Canonical energy assets'],
          ['sensor.energy_relationship_index','Connections','Physical and logical relationships'],
          ['sensor.energy_solar_property_index','Solar','Solar flow'],
          ['sensor.energy_consumption_property_index','Home demand','Household flow'],
          ['sensor.energy_grid_property_index','Grid','Import and export flow'],
          ['sensor.energy_battery_property_index','Home Battery','Home Battery charge and discharge flow'],
          ['sensor.energy_connection_property_index','Charging connections','Visible charger assignments and measured power']
        ],
        solar: [
          ['sensor.energy_release_contract','Release compatibility','Publishes the interfaces used by this screen'],
          ['sensor.energy_solar_property_index','Solar','Production and energy'],
          ['sensor.energy_forecast_property_index','Forecast','Expected production'],
          ['sensor.energy_flexible_asset_index','Flexible assets','Controllable loads and storage'],
          ['sensor.energy_planning_index','Planning facts','Targets, timing and feasibility'],
          ['sensor.energy_planning_experience_index','Planning explanation','Product-safe plan wording'],
          ['sensor.energy_command_index','Actions','Available operational commands']
        ],
        battery: [
          ['sensor.energy_release_contract','Release compatibility','Publishes the interfaces used by this screen'],
          ['sensor.energy_battery_property_index','Home Battery','Measured Home Battery state'],
          ['sensor.energy_strategy_profile_index','Configured strategy','Requested battery policy'],
          ['sensor.energy_strategy_effective_index','Effective strategy','Policy currently in effect'],
          ['sensor.energy_planning_index','Planning','Relevant storage plan'],
          ['sensor.energy_command_index','Actions','Available storage commands']
        ],
        consumers: [
          ['sensor.energy_release_contract','Release compatibility','Publishes the interfaces used by this screen'],
          ['sensor.energy_consumer_property_index','Consumers','Identity and measured facts'],
          ['sensor.energy_consumer_mix_index','Energy mix','Source contribution and attribution'],
          ['sensor.energy_flexible_asset_index','Controllability','Requested settings and readiness'],
          ['sensor.energy_value_accounting_index','Financial attribution','Consumer value and cost allocation'],
          ['sensor.energy_command_index','Actions','Meaningful available commands']
        ],
        strategies: [
          ['sensor.energy_release_contract','Release compatibility','Publishes the interfaces used by this screen'],
          ['sensor.energy_strategy_profile_index','Configured strategy','Editable strategy profiles'],
          ['sensor.energy_strategy_effective_index','Effective strategy','Policy currently applied'],
          ['sensor.energy_planning_index','Planning','Resulting structured plan'],
          ['sensor.energy_command_index','Actions','Available strategy and asset commands']
        ],
        metering: [
          ['sensor.energy_release_contract','Release compatibility','Publishes the interfaces used by this screen'],
          ['sensor.energy_metering_property_index','Measurements','Selected period, totals and quality'],
          ['sensor.energy_command_index','Reset action','Available baseline-remediation command']
        ],
        intelligence: [
          ['sensor.energy_release_contract','Release compatibility','Publishes the interfaces used by this screen'],
          ['sensor.energy_intelligence_property_index','Guidance','Recommendation, reason and affected assets'],
          ['sensor.energy_planning_experience_index','Plan explanation','Product-safe planning context'],
          ['sensor.energy_activity_index','Activity','Current operational context']
        ],
        value: [
          ['sensor.energy_release_contract','Release compatibility','Publishes the interfaces used by this screen'],
          ['sensor.energy_value_accounting_index','Financial result','Historical and current-period accounting'],
          ['sensor.energy_pricing_property_index','Pricing','Tariffs and export compensation'],
          ['sensor.energy_metering_property_index','Period context','Selected accounting period and measurements'],
          ['sensor.energy_consumer_mix_index','Consumer allocation','Energy attribution used by Value']
        ]
      };
    ...
    diagnosticsPanel(rt, tab, footer) {
      ...
      const summary = `Interfaces used by this tab: ${working}/${rows.length} available`;
      const items = rows.map(row => `<div class="hiTechnicalInterfaceRow"><span class="hiTechnicalDot ${row.working?'ok':'warn'}"></span><div><b>${escapeHtml(row.label)}</b><small>${escapeHtml(row.purpose)}</small></div><code>${escapeHtml(row.entityId)}</code><em>${escapeHtml(row.working?'Available':(row.reason || row.state || 'Unavailable'))}</em></div>`).join('');
      return `<details class="hiTechnicalFooter"><summary><span>Technical details</span><b>${escapeHtml(summary)}</b></summary><div class="hiTechnicalInterfaceList">${items}</div></details>`;
    }
    renderFooter(footer) {
      const backend = footer.backendVersion || 'Unknown';
      const contract = footer.contractVersion || 'Unknown';
      const status = footer.runtimeTrusted ? 'Runtime contract available' : 'Runtime proof not confirmed';
      return `<footer class="hiRuntimeFooter"><span>Home Intelligence Energy</span><span>UX ${escapeHtml(footer.uxVersion || UX_VERSION)}</span><span>Backend ${escapeHtml(backend)}</span><span>Contract ${escapeHtml(contract)}</span><span class="runtimeStatus">${escapeHtml(status)}</span></footer>`;
    }
...
.overviewHouseHeroImage{position:absolute;inset:0;background:linear-gradient(180deg,rgba(10,22,37,.08),rgba(10,22,37,.72)),url('/local/homebrain/infrastructure/energy/overview-hero.webp') center/cover no-repeat}
