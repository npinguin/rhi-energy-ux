...
// END GENERATED MODULE: planning/planning-view-model.js
  ...
  class HomeBrainEnergyCard extends HTMLElement {
    ...
    renderFooter(rt, tab, footer) {
      ...
      const details = [
        `UX package: ${footer.uxVersion || UX_VERSION} (HACS)`,
        `Backend release: ${backend}`,
        `Public contract: ${contract}`,
        issue ? issue.tooltip : 'Status: healthy'
      ].join('\n');
      return `<footer class="hiRuntimeFooter" title="${escapeHtml(details)}"><span>RHI Energy UX ${escapeHtml(footer.uxVersion || UX_VERSION)}</span><span>Backend ${escapeHtml(backend)}</span>${issue ? `<span class="hiReleaseIssue ${issue.severity}" title="${escapeHtml(issue.tooltip)}">${escapeHtml(issue.label)}</span>` : ''}</footer>`;
    }
