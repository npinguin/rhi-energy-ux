...
// END GENERATED MODULE: planning/planning-view-model.js
  ...
  class HomeBrainEnergyCard extends HTMLElement {
    ...
    renderFooter(rt, tab, footer) {
      ...
      return `<footer class="hiRuntimeFooter"><span>RHI Energy UX ${escapeHtml(footer.uxVersion || UX_VERSION)}</span><span>Backend ${escapeHtml(backend)}</span>${issue ? `<span class="hiReleaseIssue ${issue.severity}" title="${escapeHtml(issueDetails)}">${escapeHtml(issue.label)}</span>` : ''}</footer>`;
    }
