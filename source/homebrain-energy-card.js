(() => {
  const UX_VERSION = 'R3.94.21';
  const COMPANY_LOGO_ASSET = '/local/homebrain/infrastructure/energy/company-logo.svg';
...
// END GENERATED MODULE: planning/planning-view-model.js
  ...
  class HomeBrainEnergyCard extends HTMLElement {
    ...
    renderFooter(rt, tab, footer) {
      ...
      return `<footer class="hiRuntimeFooter rhiUxFooter" aria-label="RHI Energy release information"><span>RHI Energy UX ${escapeHtml(footer.uxVersion || UX_VERSION)}</span><span>Backend ${escapeHtml(backend)}</span>${issue ? `<span class="hiReleaseIssue rhiUxFooterIssue ${issue.severity}" title="${escapeHtml(issueDetails)}">${escapeHtml(issue.label)}</span>` : ""}</footer>`;
    }
