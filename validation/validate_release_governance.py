...
status = json.loads((ROOT / "release" / "RELEASE_STATUS.json").read_text(encoding="utf-8"))
...
shared_release = (ROOT / "docs" / "UX_RELEASE_STANDARD.md").read_text(encoding="utf-8")
shared_footer = (ROOT / "docs" / "UX_FOOTER_STANDARD.md").read_text(encoding="utf-8")
maintainability = (ROOT / "docs" / "MAINTAINABILITY.md").read_text(encoding="utf-8")
...
checks = {
    ...
    "candidate_is_hacs_visible_normal_release": "--prerelease" not in publish and "isPrerelease --jq '.isPrerelease'" in publish and "= false" in publish,
    "release_status_hacs_visible": status.get("github_release_prerelease") is False and status.get("hacs_visibility") == "normal_release_visible_without_beta_toggle",
    ...
    "shared_release_standard": "TEST CANDIDATE is published as a normal GitHub Release" in shared_release,
    "shared_footer_standard": "RHI <Module> UX <version> · Backend <version>" in shared_footer,
    "stable_promotion_evidence_only": 'gh release upload "${TAG}" release/QUALIFICATION.json --clobber' in release and "--prerelease=false" not in release,
