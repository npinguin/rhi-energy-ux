(() => {
  ...
  class HomeBrainEnergyCard extends HTMLElement {
    ...
    nav() {
      ...
      const modules = sections.map(section => ({
        ...
        items: section.items.map(item => ({
          id: item.id,
          label: item.label,
          target: `item:${section.id}:${item.id}`
        }))
      }));
      const brandHtml = `<img class="rhiEnergyCompanyLogo" src="${escapeHtml(COMPANY_LOGO_ASSET)}" alt="Robotix.be — DomotiX · Network · Security">`;
    ...
    onClick(event) {
      const coreNav = event.target.closest('[data-nav]');
      if (coreNav) {
        const [kind, sectionId, itemId] = String(coreNav.dataset.nav || '').split(':');
        if (kind === 'section' && sectionId) this.selectNavigation(sectionId);
        else if (kind === 'item' && sectionId && itemId) this.selectNavigation(sectionId, itemId);
        return;
      }
    ...
    renderFooter(rt, tab, footer) {
      ...
      const issueLines = issue
        ...
        : [];
      const sharedFooter = rhiUxTechnicalFooter({
        product: 'RHI Energy',
      ...
      const issueDetails = issue ? `
        <details class="rhiEnergyFooterDetails">
          <summary>Runtime details</summary>
          <div class="rhiEnergyFooterPanel" role="status">
            <div class="rhiEnergyFooterPanelMeta">Backend ${escapeHtml(backend)} · Contract ${escapeHtml(contract)}</div>
            ${issueLines.map(line => `<div class="rhiEnergyFooterProblem"><span class="rhiEnergyFooterProblemDot" aria-hidden="true"></span><span>${escapeHtml(line)}</span></div>`).join('')}
            <div class="rhiEnergyFooterAction">Resolve the listed runtime/backend condition, then reload this view to verify recovery.</div>
          </div>
        </details>` : '';
      return `<div class="rhiEnergyFooterWrap">${sharedFooter}${issueDetails}</div>`;
    }
