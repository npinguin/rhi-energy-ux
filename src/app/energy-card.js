(() => {
  ...
  class HomeBrainEnergyCard extends HTMLElement {
    ...
    consumerExplorerCard(rt, row = {}) {
      ...
      const asset = this.canonicalAssetFor(rt, vm?.raw || row);
    ...
    flexibleLoadMeteringTable(rows = [], periodLabel = '') {
      ...
      const renderRow = ({ row, ux }, total = false) => {
        ...
        const rt = this.runtime();
        const asset = total || ux.unattributed ? null : this.canonicalAssetFor(rt,row);
        const sourceLabel = String(row.source_label || '').trim();
        const cleanSource = sourceLabel && !/^(undefined|null|none)$/i.test(sourceLabel) ? sourceLabel : '';
        return `<tr class="${total ? 'meteringTotalRow' : ''}"><td><div class="meteringAssetIdentity">${asset ? this.assetVisual(asset,{size:'xs',fallbackIcon:this.planningAssetIcon(asset)}) : ''}<div><b>${escapeHtml(label)}</b>${cleanSource && !ux.unattributed ? `<small>${escapeHtml(cleanSource)}</small>` : ''}</div></div></td><td class="numeric">${escapeHtml(displayValue)}</td><td>${this.componentQualityChip(statusText)}</td></tr>`;
      };
    ...
    value(rt) {
      ...
      const consumers = v.consumers.map(row => {
        ...
        const id=firstDefined(item.consumer,item.consumer_id,item.asset_id,'Consumer');
        const asset=this.canonicalAssetFor(rt,item);
        const assetId=String(firstDefined(item.asset_id,item.consumer_asset_id,item.target_asset_id,'')||'');
        const label=firstDefined(item.display_name,item.asset_label,asset?.display_name,assetId?rt.assetName(assetId):'',human(id));
        return `<div class="propertyRow valueConsumerRow"><div class="valueAssetIdentity">${this.assetVisual(asset,{size:'xs',fallbackIcon:this.planningAssetIcon(asset)})}<div><b>${escapeHtml(label)}</b><span>${escapeHtml(human(firstDefined(item.attribution_quality,'Not available')))} · ${escapeHtml(fmtKwh(firstDefined(item.attributed_kwh,item.energy_kwh), 'Unavailable'))}</span></div></div><strong>${escapeHtml(money(firstDefined(item.attributed_value,item.attributed_eur)))}</strong></div>`;
      }).join('');
      ...
      const flexibleRows = this.flexibleAssetDomain(rt).participating().map(vm => { const asset = vm.raw;
        ...
        return `<div class="flexPricingRow"><div class="valueAssetIdentity">${this.assetVisual(asset,{size:'xs',fallbackIcon:this.planningAssetIcon(asset)})}<div><b>${escapeHtml(rt.assetName(id) || human(id) || 'Flexible load')}</b><span>${escapeHtml(fmtKw(power,'Inactive'))} now</span></div></div><strong>${asNumber(attributed)!==null ? escapeHtml(money(attributed)) : 'Attribution unavailable'}</strong></div>`;
      }).join('');
    ...
    canonicalAssetFor(rt, row = {}) {
      const ids = [
        row.asset_id, row.flexible_asset_id, row.target_asset_id, row.consumer_asset_id,
        row.child_asset_id, row.consumer_id, row.id
      ].map(v=>String(v||'').trim()).filter(Boolean);
      const domain = this.flexibleAssetDomain(rt);
      for (const id of ids) {
        const vm = domain.byId(id);
        if (vm?.raw) return vm.raw;
      }
      return row;
    }
...
.retroCollectingState{display:grid;grid-template-columns:64px minmax(0,1fr);gap:18px;align-items:start;padding:28px!important}.retroCollectingIcon{width:56px;height:56px;border-radius:18px;display:grid;place-items:center;background:#f5f3ff;color:#6d28d9;font-size:28px}.retroCollectingState small{display:block;color:#7c3aed;font-weight:800;letter-spacing:.12em}.retroCollectingState h2{margin:5px 0 8px!important;font-size:24px!important}.retroEvidenceSteps{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin:18px 0}.retroEvidenceSteps span{display:grid;gap:4px;padding:12px;border:1px solid #e5e7eb;border-radius:12px;background:#fafafa}.retroEvidenceSteps b{font-size:12px}.retroEvidenceSteps em{font-size:11px;color:#64748b;font-style:normal}@media(max-width:700px){.retroCollectingState{grid-template-columns:1fr}.retroEvidenceSteps{grid-template-columns:1fr}}
      ...
      .managedAssetIdentity,.strategyAssetIdentity,.meteringAssetIdentity,.valueAssetIdentity{display:flex!important;align-items:center!important;gap:10px!important;min-width:0!important}.managedAssetIdentity>div,.strategyAssetIdentity>div,.meteringAssetIdentity>div,.valueAssetIdentity>div{min-width:0!important}.managedAssetIdentity h3{margin:0!important}.meteringAssetIdentity small{display:block!important}.valueConsumerRow>div:first-child{min-width:0!important}.valueConsumerRow .valueAssetIdentity b{overflow:hidden!important;text-overflow:ellipsis!important;white-space:nowrap!important}
      .assetVisual{display:inline-flex!important;align-items:center!important;justify-content:center!important;flex:0 0 auto!important;border-radius:10px!important;background:#f5f7fa!important;overflow:hidden!important;border:1px solid #e8edf3!important}.assetVisual img{display:block!important;width:100%!important;height:100%!important;object-fit:contain!important}.assetVisual-xs{width:38px!important;height:30px!important;padding:2px!important}.assetVisual-sm{width:54px!important;height:42px!important;padding:3px!important}.assetVisual-md{width:72px!important;height:54px!important;padding:4px!important}.assetVisualFallback{font-size:18px!important;color:#52657f!important}.assetIdentityChip{display:inline-flex!important;align-items:center!important;gap:7px!important;margin:2px 8px 2px 0!important;vertical-align:middle!important}.assetIdentityChip>span:last-child{min-width:0!important}.assetIdentityChip b{display:block!important;font-size:11px!important;line-height:1.2!important}.assetIdentityChip small{display:block!important;margin-top:2px!important;font-size:9.5px!important;color:#64748b!important}.operationalSummaryCard p{display:flex!important;align-items:center!important;flex-wrap:wrap!important;gap:2px!important}.solarLoadIdentity{grid-template-columns:54px minmax(0,1fr)!important}.planningLoadIdentity{grid-template-columns:54px minmax(0,1fr)!important}.planningAssetHead{display:flex!important;align-items:center!important;gap:7px!important}.planningFooterAsset{display:inline-flex!important;align-items:center!important;gap:6px!important}.outlookChildRow{display:grid!important;grid-template-columns:38px minmax(0,1fr) auto!important;align-items:center!important;gap:8px!important}
...
.hiTabExperienceHeader{display:grid!important;grid-template-columns:minmax(0,1fr)!important;grid-auto-flow:row!important;align-items:stretch!important;width:100%!important;gap:8px!important;margin:0!important}
.hiTabHero{position:relative!important;display:block!important;min-height:clamp(176px,16vw,218px)!important;height:auto!important;padding:0!important;margin:0!important;border:0!important;border-radius:18px!important;background:linear-gradient(90deg,#fff 0%,#fff 30%,rgba(255,255,255,.94) 39%,rgba(255,255,255,.18) 60%,rgba(255,255,255,0) 76%)!important;box-shadow:none!important;overflow:hidden!important}
.hiTabHero:before,.hiTabHero:after{display:none!important}
.hiTabHeroCopy{position:relative!important;z-index:4!important;width:min(48%,650px)!important;max-width:none!important;padding:32px 20px 28px 24px!important;align-self:auto!important}
.hiTabHeroCopy>small{display:none!important}
.hiTabHeroCopy h2{font-size:clamp(31px,3.1vw,48px)!important;line-height:.98!important;letter-spacing:-.048em!important;color:#08133A!important;margin:8px 0 10px!important;font-weight:720!important}
.hiTabPurpose{display:block!important;max-width:510px!important;margin:0!important;font-size:clamp(12px,1.15vw,16px)!important;line-height:1.42!important;color:#536A91!important;font-weight:500!important;-webkit-line-clamp:unset!important;overflow:visible!important}
.hiTabLiveLine,.hiTabHeroBadge,.hiTabHeroValue{display:none!important}
.hiTabHeroArt{position:absolute!important;z-index:1!important;inset:0 0 0 27%!important;min-height:0!important;display:block!important;overflow:hidden!important;pointer-events:none!important}
.hiTabHeroArt:before{content:""!important;display:block!important;position:absolute!important;z-index:2!important;inset:0!important;background:linear-gradient(90deg,#fff 0%,rgba(255,255,255,.96) 9%,rgba(255,255,255,.68) 19%,rgba(255,255,255,.13) 37%,rgba(255,255,255,0) 55%)!important}
.hiTabHeroArt img{position:absolute!important;z-index:1!important;inset:0!important;width:100%!important;height:100%!important;min-height:0!important;max-height:none!important;object-fit:cover!important;object-position:center 52%!important;transform:none!important}
.hiTabStatusGrid{display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr))!important;gap:8px!important;margin:0!important;padding:0!important;border:0!important;border-radius:0!important;background:transparent!important;box-shadow:none!important}
.hiTabStatusItem{min-width:0!important;min-height:94px!important;height:auto!important;display:grid!important;grid-template-columns:52px minmax(0,1fr)!important;gap:11px!important;align-items:center!important;padding:12px 14px!important;border:1px solid #DBE6F3!important;border-radius:15px!important;background:rgba(255,255,255,.97)!important;box-shadow:0 8px 22px rgba(21,61,115,.045)!important}
.hiTabStatusIcon{width:46px!important;height:46px!important;min-width:46px!important;border-radius:14px!important;display:flex!important;align-items:center!important;justify-content:center!important;background:#EEF5FF!important;border:0!important;font-size:24px!important}
.hiTabStatusCopy{min-width:0!important;display:block!important}
.hiTabStatusCopy small{display:block!important;margin:0 0 3px!important;color:#31558E!important;font-size:10px!important;font-weight:650!important}
.hiTabStatusCopy b{display:block!important;margin:0 0 3px!important;color:#0B173D!important;font-size:clamp(14px,1.25vw,18px)!important;font-weight:720!important;line-height:1.08!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}
.hiTabStatusCopy em{display:block!important;margin-top:2px!important;color:#55709B!important;font-size:10px!important;font-style:normal!important;font-weight:500!important;line-height:1.2!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important}
