  ...
  class HomeBrainEnergyCard extends HTMLElement {
    ...
    solarLoadCard(rt, load, recommendation, targetId) {
      ...
      return `<article class="flexLoadCard solarLoadRow ${exceptional?'exceptional':''}">
        <div class="solarLoadSummary"><div class="solarLoadIdentity">${this.assetVisualMarkup(load,'solarLoadVisual') || `<span class="solarLoadIcon">${this.flexibleAssetIcon(load)}</span>`}<div><div class="solarLoadName"><h3>${escapeHtml(load.display_name || human(id))}</h3><span class="priorityBadge">${escapeHtml(priorityLabel)}</span></div><small><i class="dot green"></i>${escapeHtml(connection.label)}</small></div></div>
    ...
    assetVisualMarkup(asset = {}, className = 'assetVisualThumb') {
      const visual = typeof resolveEnergyVisualRef === 'function' ? resolveEnergyVisualRef(asset?.visual_ref) : null;
      return visual?.url
        ? `<span class="${escapeHtml(className)}"><img src="${escapeHtml(visual.url)}" alt="" style="filter:${escapeHtml(visual.filter || 'none')}"></span>`
        : '';
    }
    ...
    planning(rt) {
      ...
      const allAssetTotals = vm.assets.map(asset => {
        ...
        return {
          ...
          plannedEnergy: firstDefined(
            vm.horizonId === 'D1' ? asNumber(canonical.planned_tomorrow_kwh) : asNumber(canonical.planned_today_kwh),
            plannedForAsset(assetKey(asset))
          ),
